(function ($) {
    ...
    var AMOTOS_ADMIN = {
        init: function () {
            ...
            this.additionalStylingsProcess();
        },
        toolTip: function () {
        ...
        formSearchProcess: function () {
            var css_class_wrap = '.amotos-car-select-meta-box-wrap';
            var ajax_url = amotos_admin_vars.ajax_url;
            var enable_filter_location=amotos_admin_vars.enable_filter_location;
            ...
            var amotos_get_states_by_country = function () {
                ...
                var $car_state = $(".amotos-car-state-ajax", css_class_wrap);
                var $is_slug = $car_state.attr('data-slug');
                if (typeof($is_slug) === 'undefined') {
                    $is_slug='1';
                }
                if ($this.length && $car_state.length) {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            $car_state.html(response);
                            var val_selected =$car_state.attr('data-selected');
                            if (typeof val_selected !== 'undefined') {
                                $car_state.val(val_selected);
                            }
                            $this.parent().children('.amotos-loading').remove();
                        },
                        error: function () {
            ...
            var amotos_get_cities_by_state = function () {
                ...
                var $car_city = $(".amotos-car-city-ajax", css_class_wrap);
                var $is_slug = $car_city.attr('data-slug');
                if (typeof($is_slug) === 'undefined') {
                    $is_slug='1';
                }
                if ($this.length && $car_city.length) {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            $car_city.html(response);
                            var val_selected = $car_city.attr('data-selected');
                            if (typeof val_selected !== 'undefined') {
                                $car_city.val(val_selected);
                            }
                            $this.parent().children('.amotos-loading').remove();
                        },
                        error: function () {
            ...
            var amotos_get_neighborhoods_by_city = function () {
                ...
                var $car_neighborhood = $(".amotos-car-neighborhood-ajax", css_class_wrap);
                var $is_slug = $car_neighborhood.attr('data-slug');
                if (typeof($is_slug) === 'undefined') {
                    $is_slug='1';
                }
                if ($this.length && $car_neighborhood.length) {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            $car_neighborhood.html(response);
                            var val_selected = $car_neighborhood.attr('data-selected');
                            if (typeof val_selected !== 'undefined') {
                                $car_neighborhood.val(val_selected);
                            }
                            $this.parent().children('.amotos-loading').remove();
                        },
                        error: function () {
        ...
        additionalFieldProcess: function () {
            ...
            $wrapAdditionalField.on('squick_add_clone_field', function (event) {
                var $target = $(event.target);
                AMOTOS_ADMIN.additionalFieldEventProcess($target);

                $target.find('.squick-field-panel-content,.squick-clone-field-panel-inner')
            ...
            $(document).on('squick_save_option_success', function () {
                console.log('Done');
                AMOTOS_ADMIN.additionalFieldReadOnly();
            });
        },
        additionalFieldReadOnly: function() {
            $('.squick-field-panel-content,.squick-clone-field-panel-inner').each(function () {
                var $label = $(this).find('#additional_fields_label input'),
                    $id = $label.closest('.squick-field-panel-content,.squick-clone-field-panel-inner').find('#additional_fields_id input');
                if ($id.val() !== '') {
        ...
        additionalFieldEventProcess: function($wrap) {
            $wrap.find('.squick-field-panel-content,.squick-clone-field-panel-inner').find('#additional_fields_label input').on('change', function () {
                var $label = $(this),
                    $id = $label.closest('.squick-field-panel-content,.squick-clone-field-panel-inner').find('#additional_fields_id input');
                if ($id.attr('readonly') !== 'readonly') {
                    $id.val(AMOTOS_ADMIN.toSlug($label.val()));
                }
            });
        },

        additionalStylingsProcess: function() {
            var $wrap = $('#auto_moto_additional_stylings');
            ...
            $wrap.find('button').on('click', function () {
                ...
                var html = '<tr>\n' +
                    ...
                    '    </td>\n' +
                    '    <td class="title">\n' +
                    '        <input type="text" name="auto_moto_additional_styling_title[' + count +  ']" value="">\n' +
                    '    </td>\n' +
                    '    <td class="value">\n' +
                    '        <input type="text" name="auto_moto_additional_styling_value[' + count + ']" value="">\n' +
                    '    </td>\n' +
            ...
            $wrap.find('tbody').sortable({
                ...
                update: function( event, ui ) {
                    AMOTOS_ADMIN.reindexAdditionalStylings($wrap);
                },
        ...
        reindexAdditionalStylings: function($wrap) {
