...
if (!class_exists('SQUICK_Widget')) {
	class SQUICK_Widget extends WP_Widget
	{
		...
		public function update( $new_instance, $old_instance ) {
			...
			$config_keys = SQUICK()->helper()->getConfigKeys($this->settings);
		...
		public function form( $instance ) {
			if (!empty($_POST['squick_group_status'])) {
				$group_status = SQUICK()->helper()->sanitize_text($_POST['squick_group_status']);
			...
			SQUICK()->helper()->setFieldPrefix('widget-' . $this->id_base . '[' . $this->number . ']');
			SQUICK()->helper()->setFieldLayout(isset($this->settings['layout']) ? $this->settings['layout'] : 'full');
			SQUICK()->helper()->renderFields($this->settings, $instance);
			SQUICK()->helper()->setFieldPrefix('');
		}
