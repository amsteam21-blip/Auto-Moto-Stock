...
if ( ! class_exists( 'SQUICK_Core_Fonts' ) ) {
	class SQUICK_Core_Fonts {
		/*
		 * loader instances
		 */
		...
		public function do_init() {
			...
			add_action( 'wp_ajax_squick_get_font_list', array( $this, 'ajaxGetFontList' ) );
			add_action( 'wp_ajax_squick_upload_fonts', array( $this, 'ajaxUploadFonts' ) );
			add_action( 'wp_ajax_squick_delete_custom_font', array( $this, 'ajaxDeleteCustomFont' ) );
			add_action( 'wp_ajax_squick_using_font', array( $this, 'ajaxUsingFont' ) );
			add_action( 'wp_ajax_squick_remove_active_font', array( $this, 'ajaxRemoveActiveFont' ) );
			add_action( 'wp_ajax_squick_save_active_font', array( $this, 'ajaxSaveActiveFont' ) );
			add_action( 'wp_ajax_squick_reset_active_font', array( $this, 'ajaxResetActiveFont' ) );

			add_action( 'wp_ajax_squick_change_font', array( $this, 'ajaxChangeFont' ) );
		}

		public function menu() {
			$parent_slug = apply_filters('squick_admin_menu_font_management_parent_slug', 'stcore_welcome');

			add_submenu_page(
				$parent_slug,
				esc_html__( 'Fonts Management', 'auto-moto-stock' ),
				esc_html__( 'Fonts Management', 'auto-moto-stock' ),
				'manage_options',
				'stcore_fonts_management',
				array( $this, 'binderPage' )
			);

			$current_page = isset( $_GET['page'] ) ? sanitize_text_field(wp_unslash($_GET['page'])) : '';
			if ( $current_page == 'stcore_fonts_management' ) {
		...
		public function adminEnqueueStyles() {
			wp_enqueue_style( SQUICK()->assetsHandle( 'fonts' ), SQUICK()->helper()->getAssetUrl( 'core/fonts/assets/fonts.min.css' ), array('st-utils'), SQUICK()->pluginVer() );
		}

		public function adminEnqueueScripts() {
			wp_enqueue_script( SQUICK()->assetsHandle( 'fonts' ), SQUICK()->helper()->getAssetUrl( 'core/fonts/assets/fonts.min.js' ), array(
				'jquery',
				'wp-util',
				'jquery-form',
				'st-utils'
			), SQUICK()->pluginVer(), true );
			wp_localize_script( SQUICK()->assetsHandle( 'fonts' ), 'SQUICK_FONT_META_DATA', array(
				'ajaxUrl'                    => admin_url( 'admin-ajax.php' ),
				'nonce'                      => SQUICK()->helper()->getNonceValue(),
				'font_url'                   => $this->fontResoucesUrl(),
				'msgConfirmDeleteCustomFont' => esc_html__( 'Are you sure delete custom font?', 'auto-moto-stock' ),
				'msgConfirmRemoveActiveFont' => esc_html__( 'Are you sure remove this font?', 'auto-moto-stock' ),
				'msgConfirmResetFont'        => esc_html__( 'Are you sure reset active fonts?', 'auto-moto-stock' ),
			) );
		}
		...
		public function fontEnqueue() {
			...
			foreach ( $fonts['urls'] as $key => $url ) {
				wp_enqueue_style( SQUICK()->assetsHandle(sanitize_key($key )), $url, 10 );
			}
			$custom_css = $fonts['custom_css'];
			if ( ! empty( $custom_css ) ) {
				foreach ( $custom_css as $key => $value ) {
					SQUICK()->customCss()->addCss( sprintf( '%s{font-family:%s!important;}', $value, $key ) );
				}
			}
		}
		...
		public function binderPage() {
			SQUICK()->helper()->getTemplate( 'core/fonts/templates/fonts' );
		}

		public function getActiveFontsKey() {
			return apply_filters( 'squick_font_options', 'squick_font_options' );
		}

		public function getFontsDefault() {
			return apply_filters( 'squick_theme_font_default', array() );
		}
		...
		public function getFontSources() {
			return array(
				'google'   => esc_html__( 'Google Fonts', 'auto-moto-stock' ),
				'standard' => esc_html__( 'Standard Fonts', 'auto-moto-stock' ),
				'custom'   => esc_html__( 'Custom Fonts', 'auto-moto-stock' ),
			);
		}

		public function ajaxGetFontList() {
			$nonce = isset($_REQUEST['_nonce']) ? sanitize_text_field(wp_unslash($_REQUEST['_nonce'])) : '';
			if ( ! wp_verify_nonce( $nonce, 'squick_font_management' ) ) {
		...
		public function fontResoucesDir() {
			...
			return $upload_dir['basedir'] . '/squick-fonts/';
		}

		public function fontResoucesUrl() {
			$upload_dir = wp_upload_dir();

			return $upload_dir['baseurl'] . '/squick-fonts/';
		}

		public function fontsTemplates() {
			SQUICK()->helper()->getTemplate( 'core/fonts/templates/google.tpl' );
			SQUICK()->helper()->getTemplate( 'core/fonts/templates/standard.tpl' );
			SQUICK()->helper()->getTemplate( 'core/fonts/templates/custom.tpl' );
			SQUICK()->helper()->getTemplate( 'core/fonts/templates/active.tpl' );
			SQUICK()->helper()->getTemplate( 'core/fonts/templates/select-active.tpl' );
		}
		...
		private function &getGoogleFontsSource() {
			$fonts = json_decode( SQUICK()->file()->getContents( SQUICK()->pluginDir( 'assets/webfonts.json' ) ), true );
		...
		private function getCustomFonts( $is_get_default = false ) {
			$custom_fonts = get_option( 'squick_custom_fonts' );
			if ( $is_get_default ) {
		...
		private function setCustomFonts( $options ) {
			update_option( 'squick_custom_fonts', $options );
		}
		...
		public function ajaxUploadFonts() {
			...
			if ( ! file_exists( $this->fontResoucesDir() ) ) {
				SQUICK()->file()->mkdir( $this->fontResoucesDir() );
			}
			...
			if ($check_zip !== TRUE) {
				wp_send_json_error( esc_html__( 'Zip file incorrect!', 'auto-moto-stock' ) );
				die();
			}
				...
				if ( $css_count === 1 ) {
					if ( $create_font_dir ) {
						if ( @file_exists( $this->fontResoucesDir() . $sanitize_name ) ) {
							wp_send_json_error( esc_html__( 'Font Exist!', 'auto-moto-stock' ) );
							$zip->close();
                            wp_delete_file( $target_file );
							die();
						}
						SQUICK()->file()->mkdir( $this->fontResoucesDir() . $sanitize_name );
			...
			if ( $css_count === 1 ) {
				...
				$css_content = SQUICK()->file()->getContents( $this->fontResoucesDir() . $css_file );

				$count = preg_match_all( '/@font-face\s*\{([^\{]*(?=\}))\}/', $css_content, $matches );
				if ( ! $count ) {
					wp_send_json_error( esc_html__( 'Missing css file!', 'auto-moto-stock' ) );
					die();
				}
				...
				wp_send_json_error( esc_html__( 'Only allowed with 1 css file!', 'auto-moto-stock' ) );
			}
			die();
		}
		...
		public function ajaxDeleteCustomFont() {
			...
			foreach ( $option_fonts as $key => $font ) {
				if ( $font['family'] == $family_name ) {
					if ( SQUICK()->file()->delete( $this->fontResoucesDir() . $font['font_dir'], true, 'd' ) ) {
					...
					wp_send_json_error( esc_html__( 'Font directory is not deleted', 'auto-moto-stock' ) );
					die();
				}
			}
			wp_send_json_success( esc_html__( 'Delete custom font done!', 'auto-moto-stock' ) );
			die();
		}
		...
		public function ajaxUsingFont() {
			...
			$font_data           = SQUICK()->helper()->sanitize_text($_POST['font_data']);

			if (!$this->usingFont($font_data)) {
				wp_send_json_error( esc_html__( 'Fonts already exists!', 'auto-moto-stock' ) );
				die();
			}
		...
		public function ajaxSaveActiveFont() {
			...
			$fonts = SQUICK()->helper()->sanitize_text($_POST['font']);
			if ( ! array( $fonts ) ) {
				$fonts = array();
			}
			foreach ( $fonts as $key => $font ) {
			...
			$this->updateActiveFonts( $options );
			wp_send_json_success( esc_html__( 'Save Done!', 'auto-moto-stock' ) );
			die();
		}
		...
		public function ajaxChangeFont() {
			...
			$font_data           = SQUICK()->helper()->sanitize_text($_POST['font_data']);
			$from_font = SQUICK()->helper()->sanitize_text($_POST['from_font']);
			$to_font = SQUICK()->helper()->sanitize_text($_POST['to_font']);
		...
		private function replace_font_in_theme_options($from_font, $to_font) {
			$options_keys = apply_filters('squick_replace_font_option_keys', array(
				'stcore_typography_options',
				'stcore_header_options'
			));

			$preset_prefix = 'squick_preset_options_keys_';
