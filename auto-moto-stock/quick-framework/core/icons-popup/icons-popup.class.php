...
if (!class_exists('SQUICK_Core_Icons_Popup')) {
	final class SQUICK_Core_Icons_Popup
	{
		...
		public function enqueue()
		{
			...
			wp_enqueue_style(SQUICK()->assetsHandle('icons-popup'), SQUICK()->helper()->getAssetUrl('core/icons-popup/assets/icons-popup.min.css'), array('perfect-scrollbar', 'st-utils'), SQUICK()->pluginVer());
			wp_enqueue_script(SQUICK()->assetsHandle('icons-popup'), SQUICK()->helper()->getAssetUrl('core/icons-popup/assets/icons-popup.min.js'), array('jquery','perfect-scrollbar', 'st-utils'), SQUICK()->pluginVer(), true);
			wp_localize_script(SQUICK()->assetsHandle('icons-popup'), 'SQUICK_POPUP_DATA', array(
		...
		public function popup_template() {
			SQUICK()->helper()->getTemplate('core/icons-popup/templates/icons-popup.tpl');
		}

		public function get_icon_font_resources() {
			$icon_font_css = apply_filters('squick_font_icon_assets', array(
				'font-awesome' => array(
					'url' => SQUICK()->helper()->getAssetUrl('assets/vendors/font-awesome/css/font-awesome.min.css'),
					'ver' => '4.7.0',
					'deps' => array(),
				)
			));
		...
		public function icon_svg_template() {
			SQUICK()->helper()->getTemplate('core/icons-popup/templates/icons-svg.tpl');
		}
