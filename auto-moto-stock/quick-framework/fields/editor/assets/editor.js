 ...
 * Define class field
 */
var SQUICK_EditorClass = function($container) {
...
(function($) {
	...
	SQUICK_EditorClass.prototype = {
		...
		initNotExistEditor: function () {
			if (this.$container.find('.mce-tinymce').length == 0) {
				...
				if (!$txt.closest('.squick-field-clone-item').length) {
					return;
				}
		...
		onChange: function (i) {
			if (tinymce.editors[i].isChangeEvent == null) {
				...
				tinymce.editors[i].on('change', function (e) {
					if (e.lastLevel != null) {
						this.save();
						$(this.container).closest('.squick-field-content-inner').find('.wp-editor-area').trigger('squick_field_control_changed');
					}
				});
			}
				...
				function() {
					if (typeof(tinymce) !== 'undefined') {
						for ( var i = 0; i < tinymce.editors.length; i++ ) {
							SQUICK_EditorObject.onChange(i);
						}
					}
				}, 1000
			);
	...
	$(document).on('squick_add_clone_field', function(event) {
		...
		if ($container.find('.squick-field-editor-inner').length > 0) {
			SQUICK_EditorObject.cloneItem($(event.target));
			SQUICK_EditorObject.onChangeEditor();
		}
	});
