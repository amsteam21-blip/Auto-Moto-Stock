...
if (!class_exists('SQUICK_Field_Panel')) {
	class SQUICK_Field_Panel extends  SQUICK_Field {
		...
		public function renderCloneRemoveButton() {
			?>
			<span class="squick-clone-button-remove"><i class="dashicons dashicons-no-alt"></i></span>
		<?php
		}

		public function renderPanelTitle() {
			$title_class = array('squick-field-panel-title');
			if ($this->isSort()) {
				$title_class[] = 'squick-field-panel-title-sortable';
			}
			?>
			<h4 class="<?php echo esc_attr(join(' ', $title_class)); ?>">
				<span class="squick-field-panel-title-text"><?php echo esc_html($this->_setting['title']); ?></span>
				<?php if ($this->isClone()): ?>
					<?php $this->renderCloneRemoveButton(); ?>
				<?php endif;?>
				<span class="squick-field-panel-title-toggle"></span>
			</h4>
			<?php
		}
		...
		public function renderContentWrapper() {
			...
			<div class="squick-field-content">
				<?php if ($this->isClone()): ?>
					...
					?>
					<div class="squick-field-clone-wrapper squick-field-panel-clone <?php echo esc_attr($this->isSort() ? 'squick-field-clone-sortable' : ''); ?>">
						<?php for ($index = 0; $index < $count_clone; $index++): ?>
							<div class="squick-field-clone-item squick-field-panel-clone-item" data-clone-index="<?php echo esc_attr($index); ?>">
								<div class="squick-field-content-inner squick-field-panel-inner" data-panel-title="<?php echo esc_attr($panel_title); ?>">
		...
		public function renderCloneAddButton() {
			?>
			<button class="squick-clone-button-add button"
			        type="button"><?php echo esc_html__('+ Add ', 'auto-moto-stock'); ?> <?php echo esc_html($this->_setting['title']); ?></button>
		<?php
		}

		public function renderPanelContent($panel_index) {
			?>
			<div class="squick-field-panel-content">
				<?php
				...
				?>
			</div><!-- /.squick-field-panel-content -->
		<?php
		}
