...
if (!class_exists('SQUICK_Field_Typography')) {
	class SQUICK_Field_Typography extends SQUICK_Field
	{
		function enqueue()
		{
			...
			wp_enqueue_script(SQUICK()->assetsHandle('field_typography'), SQUICK()->helper()->getAssetUrl('fields/typography/assets/typography.min.js'), array(), SQUICK()->pluginVer(), true);
			wp_enqueue_style(SQUICK()->assetsHandle('field_typography'), SQUICK()->helper()->getAssetUrl('fields/typography/assets/typography.min.css'), array(), SQUICK()->pluginVer());
			wp_localize_script(SQUICK()->assetsHandle('field_typography'), 'SQUICK_TYPOGRAPHY_META_DATA', array(
		...
		function renderContent()
		{
			...
			?>
			<div class="squick-field-typography-inner squick-clearfix">
				<div class="squick-typography-field squick-typography-family">
					<div class="squick-typography-label"><?php esc_html_e('Font Family', 'auto-moto-stock'); ?></div>
					<select data-field-control="" placeholder="<?php esc_attr_e('Select Font Family', 'auto-moto-stock'); ?>"
					        class="squick-typography-font-family"
						...
						<?php if (!in_array($field_value['font_family'], $list_font_available)): ?>
							<option value="<?php echo esc_attr($field_value['font_family']); ?>">
								...
								<?php echo esc_html__('(Not activated)', 'auto-moto-stock') ?></option>
						<?php endif; ?>
				...
				<?php if (isset($this->_setting['font_size']) && $this->_setting['font_size']): ?>
					<div class="squick-typography-field squick-typography-size">
						<div class="squick-typography-label"><?php esc_html_e('Font Size', 'auto-moto-stock'); ?></div>
						<input data-field-control="" type="hidden" class="squick-typography-size"
						       name="<?php $this->theInputName(); ?>[font_size]"
						       value="<?php echo esc_attr($field_value['font_size']); ?>"/>
						<input type="number" placeholder="<?php esc_attr_e('Font Size', 'auto-moto-stock'); ?>"
						       class="squick-typography-size-value" value="<?php echo esc_attr($font_size_value); ?>" step="0.001"/>
						<select class="squick-typography-size-unit">
				...
				<?php if (isset($this->_setting['font_variants']) && $this->_setting['font_variants']): ?>
					<div class="squick-typography-field squick-typography-weight-style">
						<input data-field-control="" type="hidden" class="squick-typography-weight"
						       name="<?php $this->theInputName(); ?>[font_weight]"
						       value="<?php echo esc_attr($field_value['font_weight']); ?>"/>
						<input data-field-control="" type="hidden" class="squick-typography-style"
						       name="<?php $this->theInputName(); ?>[font_style]"
						       value="<?php echo esc_attr($field_value['font_style']); ?>"/>
						<div class="squick-typography-label"><?php esc_html_e('Font Weight & Style', 'auto-moto-stock'); ?></div>
						<?php $current_variant = $field_value['font_weight'].$field_value['font_style']; ?>
						<select class="squick-typography-variants">
							<?php if (!isset($current_font['variants']) || !in_array($current_variant, $current_font['variants'])): ?>
								<option value="<?php echo esc_attr($current_variant) ?>">
									...
									<?php echo esc_html__('(Missing variant)', 'auto-moto-stock') ?>
								</option>
							<?php endif; ?>
				...
				<?php if (isset($this->_setting['align']) && $this->_setting['align']): ?>
                    <div class="squick-typography-field">
	                    <?php
	                    $aligns = array(
		                    'inherit' => 'Inherit',
		                    'left'    => 'Left',
	                    ...
	                    ?>
                        <div class="squick-typography-label"><?php esc_html_e('Align', 'auto-moto-stock'); ?></div>
				...
				<?php if (isset($this->_setting['transform']) && $this->_setting['transform']): ?>
                    <div class="squick-typography-field">
						...
						?>
                        <div class="squick-typography-label"><?php esc_html_e('Text Transform', 'auto-moto-stock'); ?></div>
				...
				<?php if (isset($this->_setting['line_height']) && $this->_setting['line_height']): ?>
                    <div class="squick-typography-field">
                        <div class="squick-typography-label"><?php esc_html_e('Line Height', 'auto-moto-stock'); ?></div>
                        <input data-field-control="" name="<?php $this->theInputName(); ?>[line_height]" type="number"  step="0.001"
                               class="squick-typography-line-height-value" value="<?php echo esc_attr($field_value['line_height']); ?>"/>
                    </div>
				<?php endif;?>

				<?php if (isset($this->_setting['letter_spacing']) && $this->_setting['letter_spacing']): ?>
                    <div class="squick-typography-field">
                        <div class="squick-typography-label"><?php esc_html_e('Letter Spacing', 'auto-moto-stock'); ?></div>
                        <input data-field-control="" name="<?php $this->theInputName(); ?>[letter_spacing]" type="number"  step="0.001"
                               class="squick-typography-letter-spacing-value" value="<?php echo esc_attr($field_value['letter_spacing']); ?>"/>
                    </div>
				<?php endif;?>
				...
				<?php if (isset($this->_setting['color']) && $this->_setting['color']): ?>
                    ...
                    <div class="squick-typography-field">
                        <div class="squick-typography-label"><?php esc_html_e('Color', 'auto-moto-stock'); ?></div>

                        <input class="squick-typography-color"
                               data-field-control=""
                               type="text"
		...
		private function getActiveFonts() {
			return SQUICK_Core_Fonts::getInstance()->getActiveFonts();
		}
	}
}