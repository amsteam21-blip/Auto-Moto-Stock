...
if (!class_exists('SQUICK_Field_Image_Set')) {
	class SQUICK_Field_Image_Set extends SQUICK_Field
	{
		function enqueue() {
			wp_enqueue_style(SQUICK()->assetsHandle('field_image_set'));
			wp_enqueue_script(SQUICK()->assetsHandle('field_image_set'), SQUICK()->helper()->getAssetUrl('fields/image_set/assets/image-set.min.js'), array(), SQUICK()->pluginVer(), true);
		}
		function renderContent()
		{
			...
			if (isset($this->_setting['width']) && !empty($this->_setting['width'])) {
				$img_attr['width'] = $this->_setting['width'];
			}
			if (isset($this->_setting['height']) && !empty($this->_setting['height'])) {
				$img_attr['height'] = $this->_setting['height'];
			}

			$field_attrs = array();
			if ($is_multiple) {
                $field_attrs['data-multiple'] = 'true';
			}
			?>
			<div class="squick-field-image_set-inner" <?php SQUICK()->helper()->render_html_attr($field_attrs); ?>>
				<?php foreach ($this->_setting['options'] as $key => $value): ?>
					...
					<label>
						...
						?>
						<img class="<?php echo esc_attr($allowClear ? 'squick-allow-clear' : ''); ?>"
						     src="<?php echo esc_url($img_src); ?>"
							<?php SQUICK()->helper()->render_html_attr($img_attr); ?>
