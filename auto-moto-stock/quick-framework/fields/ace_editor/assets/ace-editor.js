...
var SQUICK_Ace_editorClass = function($container) {
...
(function($) {
	 ...
	 */
	SQUICK_Ace_editorClass.prototype = {
		init: function() {
			this.$fieldText = this.$container.find('textarea');
			this.$editorField = this.$container.find('.squick-ace-editor');
			...
			this.editor.on('change', function (event) {
				...
				var $field = self.$container.closest('.squick-field');
				$field.find('[data-field-control]').trigger('squick_field_control_changed');
			});
		},
		getValue: function() {
