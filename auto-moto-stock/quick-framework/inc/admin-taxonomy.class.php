...
if (!class_exists('SQUICK_Inc_Admin_Taxonomy')) {
	class SQUICK_Inc_Admin_Taxonomy
	{
		/*
		 * loader instances
		 */
		...
		public function init() {
			...
			add_action('wp_ajax_squick_tax_meta_form', array($this, 'ajaxMetaForm'));
		...
		public function adminEnqueueStyles() {
			...
			wp_enqueue_style(SQUICK()->assetsHandle('term-meta'));
		...
		public function adminEnqueueScripts() {
			...
			wp_enqueue_script(SQUICK()->assetsHandle('term-meta'));
		...
		public function registerCustomTaxonomy() {
			$custom_tax = apply_filters('squick_register_taxonomy', array());
			foreach ($custom_tax as $tax => $args) {
				...
				$default = array(
					...
					'labels'       => array(
						...
						'search_items'               => sprintf(esc_html__('Search %s', 'auto-moto-stock'), $label),
                        /* translators: %s: label of taxonomy */
						'popular_items'              => sprintf(esc_html__('Popular %s', 'auto-moto-stock'), $label),
                        /* translators: %s: label of taxonomy */
						'all_items'                  => sprintf(esc_html__('All %s', 'auto-moto-stock'), $label),
                        /* translators: %s: singular name of taxonomy */
						'parent_item'                => sprintf(esc_html__('Parent %s', 'auto-moto-stock'), $singular_name),
                        /* translators: %s: singular name of taxonomy */
						'parent_item_colon'          => sprintf(esc_html__('Parent %s:', 'auto-moto-stock'), $singular_name),
                        /* translators: %s: singular name of taxonomy */
						'edit_item'                  => sprintf(esc_html__('Edit %s', 'auto-moto-stock'), $singular_name),
                        /* translators: %s: singular name of taxonomy */
						'view_item'                  => sprintf(esc_html__('View %s', 'auto-moto-stock'), $singular_name),
                        /* translators: %s: singular name of taxonomy */
						'update_item'                => sprintf(esc_html__('Update %s', 'auto-moto-stock'), $singular_name),
                        /* translators: %s: singular name of taxonomy */
						'add_new_item'               => sprintf(esc_html__('Add New %s', 'auto-moto-stock'), $singular_name),
                        /* translators: %s: singular name of taxonomy */
						'new_item_name'              => sprintf(esc_html__('New %s New', 'auto-moto-stock'), $singular_name),
                        /* translators: %s: label of taxonomy */
						'separate_items_with_commas' => sprintf(esc_html__('Separate %s with commas', 'auto-moto-stock'), strtolower($label)),
                        /* translators: %s: label of taxonomy */
						'add_or_remove_items'        => sprintf(esc_html__('Add or remove %s', 'auto-moto-stock'), strtolower($label)),
                        /* translators: %s: label of taxonomy */
						'choose_from_most_used'      => sprintf(esc_html__('Choose from the most used %s', 'auto-moto-stock'), strtolower($label)),
                        /* translators: %s: label of taxonomy */
						'not_found'                  => sprintf(esc_html__('No %s found.', 'auto-moto-stock'), strtolower($label)),
                        /* translators: %s: label of taxonomy */
						'no_terms'                   => sprintf(esc_html__('No %s', 'auto-moto-stock'), strtolower($label)),
                        /* translators: %s: label of taxonomy */
						'items_list_navigation'      => sprintf(esc_html__('%s list navigation', 'auto-moto-stock'), $label),
                        /* translators: %s: label of taxonomy */
						'items_list'                 => sprintf(esc_html__('%s list', 'auto-moto-stock'), $label),
					)
				);
		...
		public function ajaxMetaForm() {
            ...
            check_ajax_referer('squick_term_meta');

		    if (isset($_REQUEST['taxonomy'])) {
			    $taxonomy = SQUICK()->helper()->sanitize_text($_REQUEST['taxonomy']);
		...
		public function termMetaAddDisplay($taxonomy) {
			?>
			<div class="squick-term-meta-wrapper squick-term-meta-add-new" data-nonce="<?php echo esc_attr(wp_create_nonce('squick_term_meta')) ?>" data-taxonomy="<?php echo esc_attr($taxonomy); ?>">
		...
		public function termMetaEditDisplay($term, $taxonomy) {
			...
			<tr class="form-field">
				<td colspan="2" style="padding: 0">
					<div class="squick-term-meta-wrapper squick-term-meta-edit" data-nonce="<?php echo esc_attr(wp_create_nonce('squick_term_meta')) ?>">
		...
		public function termMetaDisplay($taxonomy, $is_new) {
			...
			foreach ($meta_configs as $meta_id => $configs) {
				...
				if (in_array($taxonomy, $taxonomies)) {
					?>
					<div class="squick-taxonomy-meta-wrapper">
						<div class="squick-taxonomy-meta-header">
							<h4>
								<span class="squick-taxonomy-meta-header-title"><?php echo esc_html($configs['name']); ?></span>
								<button type="button" class="squick-taxonomy-meta-header-toggle">
									<span></span>
								</button>
							</h4>
						</div>
						<div class="squick-taxonomy-meta-content">
							<?php
		...
		public function updateTermMeta($term_id, $tag_id) {
            ...
            $current_taxonomy = SQUICK()->helper()->sanitize_text($_POST['taxonomy']);
		...
		public function saveTermMeta($term_id, $tag_id) {
			...
			foreach ($meta_field_keys as $meta_id => $field_meta) {
				$meta_value = isset($_POST[$meta_id])
                    ? SQUICK()->helper()->sanitize_text($_POST[$meta_id])
		...
		public function &getMetaConfig() {
			if (!isset($GLOBALS['squick_term_meta_config'])) {
				$GLOBALS['squick_term_meta_config'] = apply_filters('squick_term_meta_config', array());
			}
			return $GLOBALS['squick_term_meta_config'];
		}

		private function getMetaValue(&$configs) {
			$id = isset($_GET['tag_ID']) ? SQUICK()->helper()->sanitize_text($_GET['tag_ID']) : '0';
			$meta_values = array();
		...
		private function isTermPage() {
			$taxonomy = isset($_REQUEST['taxonomy']) ? SQUICK()->helper()->sanitize_text($_REQUEST['taxonomy']) : '';
			$action = isset($_REQUEST['action']) ? SQUICK()->helper()->sanitize_text($_REQUEST['action']) : '';
			if (empty($action)) {
				$screen = SQUICK_Inc_Helper::getInstance()->get_current_screen();
				$action = $screen->base;
			}
