...
if (!class_exists('SQUICK_Inc_Helper')) {
	class SQUICK_Inc_Helper
	{
		...
		public function createField($type) {
			...
			$class_name = 'SQUICK_Field_' . $class_name;
		...
		public function setFieldPrefix($prefix) {
			$GLOBALS['squick_field_prefix'] = $prefix;
		}

		public function getFieldPrefix() {
			return isset($GLOBALS['squick_field_prefix']) ? $GLOBALS['squick_field_prefix'] : '';
		}
		...
		public function setFieldLayout($layout) {
			...
			$GLOBALS['squick_field_layout'] = $layout;
		}
		...
		public function getFieldLayout() {
			if (!isset($GLOBALS['squick_field_layout'])) {
				$GLOBALS['squick_field_layout'] = 'inline';
			}
			return $GLOBALS['squick_field_layout'];
		}
		...
		public function getTemplate($slug, $args = array()) {
			$located = SQUICK()->pluginDir($slug . '.php');
            $action_args = array(
                'located'       => $located,
            );
		...
		public function getAssetUrl($file) {
			if (!file_exists(SQUICK()->pluginDir($file)) || (defined('SCRIPT_DEBUG') && SCRIPT_DEBUG)) {
				$ext = explode('.', $file);
				$ext = end($ext);
				$normal_file = preg_replace('/((\.min\.css)|(\.min\.js))$/', '', $file);
				if ($normal_file != $file) {
					$normal_file = untrailingslashit($normal_file) . ".{$ext}";
					if (file_exists(SQUICK()->pluginDir($normal_file))) {
						return SQUICK()->pluginUrl(untrailingslashit($normal_file));
					}
				}
			}
			return SQUICK()->pluginUrl(untrailingslashit($file));
		}
		...
		public function renderFields(&$config, &$values, $current_preset = '') {
			...
			if (!empty($config)) {
				if (isset($config['section'])) {
					?>
					<?php if (SQUICK()->adminThemeOption()->is_theme_option_page): ?>
						<?php
						...
						<?php foreach ($config['section'] as &$section): ?>
							<?php if ($section_current_id === $section['id']): ?>
								<div id="section_<?php echo esc_attr($section['id']) ?>" class="squick-section-container">
									<h4 class="squick-section-title">
										<i class="squick-section-title-icon <?php echo esc_attr(isset($section['icon']) ? $section['icon'] : 'dashicons dashicons-admin-generic'); ?>"></i>
										<span><?php echo esc_html($section['title']); ?></span>
										<span class="squick-section-title-toggle"></span>
									</h4>
									<div class="squick-section-inner">
										<?php if (isset($section['fields'])): ?>
											<?php $this->renderSubFields($section['fields'], $values) ?>
										<?php endif; ?>
									</div>
								</div><!-- /.squick-section-container  -->
								<?php break; ?>
							<?php endif; ?>
						<?php endforeach; ?>
					<?php else: ?>
		...
		public function getNonceVerifyKey() {
			return apply_filters('squick_nonce_verify_key', 'SQUICK_QUICK_FRAMEWORK_VERIFY');
		}
		...
		public function getFontIcons() {
			return apply_filters('squick_font_icon_config', array(
				'font-awesome' => array(
					'label' => esc_html__('Font Awesome', 'auto-moto-stock'),
					'total' => '786',
		...
		public function getFontIconsSvg() {
			return apply_filters('squick_font_icon_svg', array());
		}
