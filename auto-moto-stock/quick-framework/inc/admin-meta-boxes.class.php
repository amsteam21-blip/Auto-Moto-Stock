...
if (!class_exists('SQUICK_Inc_Admin_Meta_Boxes')) {
    class SQUICK_Inc_Admin_Meta_Boxes
    {
        /*
         * loader instances
         */
        ...
        public function registerCustomPostType() {
            $ctp_args = apply_filters('squick_register_post_type', array());
            foreach ($ctp_args as $post_type => $args) {
                ...
                $defaults = array(
                    ...
                    'labels'             => array(
                        ...
                        'add_new_item'          => sprintf(__('Add New %s', 'auto-moto-stock'), $singular_name),
                        /* translators: %s: singular name of post type */
                        'edit_item'             => sprintf(__('Edit %s', 'auto-moto-stock'), $singular_name),
                        /* translators: %s: singular name of post type */
                        'new_item'              => sprintf(__('New %s', 'auto-moto-stock'), $singular_name),
                        /* translators: %s: singular name of post type */
                        'view_item'             => sprintf(__('View %s', 'auto-moto-stock'), $singular_name),
                        /* translators: %s: post type name */
                        'search_items'          => sprintf(__('Search %s', 'auto-moto-stock'), $post_type_name),
                        /* translators: %s: post type name */
                        'not_found'             => sprintf(__('No %s found.', 'auto-moto-stock'), strtolower($post_type_name)),
                        /* translators: %s: post type name */
                        'not_found_in_trash'    => sprintf(__('No %s found in Trash.', 'auto-moto-stock'), strtolower($post_type_name)),
                        /* translators: %s: post type name */
                        'all_items'             => sprintf(__('All %s', 'auto-moto-stock'), $post_type_name),
                        /* translators: %s: post type name */
                        'archives'              => sprintf(__('%s Archives', 'auto-moto-stock'), $post_type_name),
                        /* translators: %s: singular name of post type */
                        'insert_into_item'      => sprintf(__('Insert into %s', 'auto-moto-stock'), strtolower($singular_name)),
                        /* translators: %s: singular name of post type */
                        'uploaded_to_this_item' => sprintf(__('Uploaded to this %s', 'auto-moto-stock'), strtolower($singular_name)),
                        /* translators: %s: post type name */
                        'filter_items_list'     => sprintf(__('Filter %s list', 'auto-moto-stock'), strtolower($post_type_name)),
                        /* translators: %s: post type name */
                        'items_list_navigation' => sprintf(__('%s list navigation', 'auto-moto-stock'), $post_type_name),
                        /* translators: %s: post type name */
                        'items_list'            => sprintf(__('%s list', 'auto-moto-stock'), $post_type_name),
                    )
                );
        ...
        public function adminEnqueueScripts() {
            ...
            //wp_enqueue_script(SQUICK()->assetsHandle('meta-box'));
        ...
        public function &getMetaConfig() {
            if (!isset($GLOBALS['squick_meta_box_config'])) {
                $GLOBALS['squick_meta_box_config'] = apply_filters('squick_meta_box_config', array());
            }
            return $GLOBALS['squick_meta_box_config'];
        }
        ...
        public function metaBoxDisplayCallback($post, $args) {
            ...
            wp_nonce_field('squick_meta_box','squick_meta_box');
        ...
        public function saveMetaBoxes($post_id) {
            ...
            if (!isset($_REQUEST['squick_meta_box']) || !wp_verify_nonce(wp_unslash($_REQUEST['squick_meta_box']),'squick_meta_box')) {
                return;
            }
	        ...
	        $meta_field_keys = apply_filters('squick_save_metabox_meta_field_keys', $meta_field_keys, $post_id, $current_post_type);
