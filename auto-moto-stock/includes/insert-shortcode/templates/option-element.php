...
<div class="option-item-wrap <?php echo esc_attr($name) ?>">
    ...
    <?php if ($attr_option['type'] === 'checkbox'): ?>
        <div class="content">
            <input  name="<?php echo esc_attr($name) ?>" type="checkbox"
                    ...
                    id="<?php echo esc_attr($name) ?>" <?php amotos_render_html_attr($input_attrs); ?>/>
            <?php echo wp_kses_post($desc) ?>
        </div>
    <?php elseif ($attr_option['type'] === 'select'): ?>
        <div class="content">
            <select id="<?php echo esc_attr($name) ?>" name="<?php echo esc_attr($name) ?>" <?php amotos_render_html_attr($input_attrs); ?>>
	...
	<?php elseif ($attr_option['type'] === 'amotos_selectize'): ?>
        <div class="content">
            <select class="amotos-selectize-input" multiple="multiple" id="<?php echo esc_attr($name) ?>" name="<?php echo esc_attr($name ) ?>" <?php amotos_render_html_attr($input_attrs); ?>>
	            <?php foreach ( $attr_option['values'] as $key => $value ): ?>
                    <option value="<?php echo esc_attr($key) ?>"><?php echo esc_html($value) ?></option>
	            <?php endforeach; ?>
            </select>
	...
	<?php elseif ($attr_option['type'] === 'image'): ?>
        <div class="content">
            <div class="shortcode-dynamic-item" id="options-item" data-name="image-upload">
                ...
                <img class="amotos-image-screenshot" id="<?php echo esc_attr($name) ?>" src="" />
                <a data-update="Select File" data-choose="Choose a File" href="javascript:void(0);" class="amotos-image-upload button-secondary" rel-id="">
				    <?php echo esc_html__( 'Upload', 'auto-moto-stock' ) ?>
                </a>
                <a href="javascript:void(0);" class="amotos-image-upload-remove" style="display: none;"><?php echo esc_html__( 'Remove Upload', 'auto-moto-stock' ) ?></a>
    ...
    <?php else: ?>
        <div class="content">
            <input id="<?php echo esc_attr($name) ?>" type="text" name="<?php echo esc_attr($name) ?>" value="" <?php amotos_render_html_attr($input_attrs); ?> />
            <?php echo wp_kses_post($desc) ?>
        </div>
	<?php endif; ?>
</div>