var AMOTOS_POPUP = AMOTOS_POPUP || {};
(function ($) {
    AMOTOS_POPUP = {
        init: function () {
            //The chosen one
            $("select#amotos-shortcodes").selectize();

            this.insert_shortcode();
            this.select_shortcode();
        },
        required_element: function() {
            var shortcode_option = $('#options-'+$('#amotos-shortcodes').val());
        ...
        update_shortcode: function(){
            var name = $('#amotos-shortcodes').val(),
                code = '['+name;
        ...
        insert_shortcode: function () {
            $('#insert-shortcode').on('click', function(){
                AMOTOS_POPUP.update_shortcode();
                return false;
            });
        },
        select_shortcode: function () {
            $('#amotos-shortcodes').on('change', function(){
        ...
        reset_fileds: function(){
            //reset data
            var wrap = $('#amotos-input-shortcode'),
                shortcode = wrap.find('#amotos-shortcodes').val(),
                current_shortcode = $('#options-'+shortcode);
            //current_shortcode.find('');
            current_shortcode.find('.amotos-selectize-input').selectize({
            ...
            current_shortcode.find('select:not("#amotos-shortcodes, [multiple=multiple]")').each(function () {
                var $this = $(this),
                    default_value = $this.attr('data-default-value');
            ...
            current_shortcode.find('.amotos-image-screenshot').attr('src','');
            current_shortcode.find('.amotos-image-upload-remove').hide();
            current_shortcode.find('.amotos-image-upload').show();
            current_shortcode.find('.amotos-selectize-input').each(function(){
                if ($(this)[0].selectize) {
