var AMOTOS_Widget_Acf = AMOTOS_Widget_Acf || {};
(function($){
	"use strict";
	AMOTOS_Widget_Acf = {
		templates : [],
		init : function() {
			...
			this.deleteSection();
			this.initControls();
			$(document).on('widget_acf_make_template_done',AMOTOS_Widget_Acf.initControls);
			$(document).on('widget-added', AMOTOS_Widget_Acf.makeTemplate);
			$(document).on('widget-updated', AMOTOS_Widget_Acf.makeTemplate);
		},
		makeTemplate: function(event, widget){
			...
			var index = 0;
			AMOTOS_Widget_Acf.templates = [];
			$('.accordion-wrap',$wrapper).each(function(){
				var clone = $('.widget_acf_accordion',this).last();
				if (clone.length) {
					AMOTOS_Widget_Acf.templates[index] = clone[0].outerHTML;
		...
		sortable : function($wrap) {
			if ($.isFunction($.fn.sortable)){
				$('.accordion-wrap',$wrap).each(function(){
					...
					$this.sortable({
						update: function(){
							AMOTOS_Widget_Acf.reIndexSection($this);
						}
					});
				});
			}
		...
		addSection : function() {
			$(document).on('click','.widget_acf_wrap .button.add',function(){
				...
				var $element = $(AMOTOS_Widget_Acf.templates[templateIndex]);
				$section_wrap.append($element);
				AMOTOS_Widget_Acf.reIndexSection($section_wrap);
				AMOTOS_Widget_Acf.initControls(null,$element);
			});
		},
		deleteSection : function(){
			$(document).on('click','.widget_acf_accordion .button.deletion',function(){
				...
				if ($items > 1) {
					...
					AMOTOS_Widget_Acf.reIndexSection($wrap);
				} else {
		...
		registerRequireElement : function($wrap) {
			$('input, select', $wrap).each(function() {
				AMOTOS_Widget_Acf.initRequireElement($(this));
		...
		uploadImage: function () {
			$('.widget-acf-upload-button').each(function () {
				$(this).off('click').on('click',function(event){
					...
					var gk_media_set_image = function () {
						...
						selection.each(function (attachment) {
							...
							AMOTOS_Widget_Acf.removeImage(parent);
						});
					};
				...
				AMOTOS_Widget_Acf.removeImage($(this).parent());
			});
		},
		...
		initControls : function(event, widget){
			...
			AMOTOS_Widget_Acf.sortable($wrapper);
			AMOTOS_Widget_Acf.initCheckbox($wrapper);
			AMOTOS_Widget_Acf.initSelectize($wrapper);
			AMOTOS_Widget_Acf.registerRequireElement($wrapper);
		}
	};

	$(document).ready(function(){
		AMOTOS_Widget_Acf.init();
	});
})(jQuery);