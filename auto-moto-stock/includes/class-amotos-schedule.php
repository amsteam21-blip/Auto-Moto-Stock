...
if (!class_exists('AMOTOS_Schedule')) {
	/**
	 * Class AMOTOS_Schedule
	 */
	class AMOTOS_Schedule
	{
		...
		*/
		...
		public function per_listing_check_expire()
		{
			...
			$number_expire_days = intval(amotos_get_option('number_expire_days',30));
			if ($number_expire_days > 0 && $per_listing_expire_days == 1) {
				...
				while ($data->have_posts()): $data->the_post();
					$post_id = get_the_ID();
					$car_date = strtotime(get_the_date("Y-m-d H:i:s", $post_id));
					$expired_date = $car_date + $number_expire_days * 24 * 60 * 60;
					$today = time();
					...
					if ($user_role != 'administrator') {
						if ($expired_date < $today) {
							...
							wp_update_post($args);
							update_post_meta($post_id, AMOTOS_METABOX_PREFIX . 'car_featured', 0);
							update_post_meta($post_id, AMOTOS_METABOX_PREFIX . 'payment_status', 'not_paid');
		...
		public function search_query_results($search_query){
			...
			if($data->have_posts()){
				while ($data->have_posts()): $data->the_post();
					$car_id = get_the_ID();
					$links .= get_the_permalink($car_id)."\r\n";
				endwhile;
				wp_reset_postdata();
			}
			return $links;
				...
				if ( sizeof ( $results ) !== 0 ){
					foreach ( $results as $result ){
						...
						if ($links != '') :
							...
							amotos_send_email($user_email, 'mail_matching_saved_search', $args);
						endif;
					}
				}
				wp_reset_postdata();
		...
		public function scheduled_hook()
		{
			...
			if ($paid_submission_type == 'per_listing') {
				...
				if ($per_listing_expire_days == 1) {
					if (!wp_next_scheduled('amotos_per_listing_check_expire')) {
						//twicedaily
						wp_schedule_event(time(), 'twicedaily', 'amotos_per_listing_check_expire');
					}
				}
			}
			...
			{
				if (!wp_next_scheduled('amotos_saved_search_check_result')) {
					wp_schedule_event(time(), 'daily', 'amotos_saved_search_check_result');
				}
			}
		}
		...
		{
			wp_clear_scheduled_hook('amotos_per_listing_check_expire');
			wp_clear_scheduled_hook('amotos_saved_search_check_result');
			do_action('amotos_clear_scheduled_hook');
		}
	}
}